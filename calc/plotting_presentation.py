...
def plot_data(dataframe, filename, ylistticks, Y_label, tuple_figsize):
    """"""

    ax=dataframe.plot(kind="bar", fontsize=12, yticks=ylistticks, figsize=tuple_figsize)
    filename = "figures/pr_" + filename
    for p in ax.patches:
        ax.annotate(str(p.get_height()), (p.get_x() * 0.98, p.get_height() * 1), fontSize=12)

    plt.ylabel(Y_label, size=12)
    plt.xlabel('Зразок', size=12)
...
pgf_with_custom_preamble = {
    ...
    "pgf.preamble": [
        ...
        r"\setmathfont(Digits,Latin,Greek)[Scale=0.976, Numbers={Lining,Proportional}]{Arial}",
        r"\setmainfont[Scale=0.976]{Arial}",  # serif font via preamble
        ]
}
...
for sample in samples.values():
    ...
    error = 0.0005
    pltname = 'figures/plt_pr_gravi_' + sample
    df_grav.plot(yerr=error)
    plt.title(r'\textbf{%s}' % sample, size=14)
    plt.legend()
    plt.axhline(y=0, linewidth=1, color='k')
    plt.grid()
    plt.xticks(list(range(11)), size=14)
    plt.yticks(y_ticks, size=12)
    plt.ylabel(r'Ерозія $\sum \Delta m$, $\frac{\text{г}}{\text{см}^2}$', size=14)
    ...
    # PLOT HARDNESS
    df.plot.scatter(x='depth', y=sample, color='k', yerr=0.5, xerr=2.5)
    plt.grid()
    plt.title(r'\textbf{%s}' % sample, size=14)
    plt.ylabel('Мікротвердість, ГПа', size=14)
    plt.xlabel('Відстань від поверхні, мкм', size=14)
    plt.yticks(size=14)
    plt.xticks(size=14)
    plt.tight_layout(0)
    plt.savefig("figures/plt_pr_hard_" + sample + ".pdf", format="pdf")
    plt.clf()

    # PLOT WEAR GRAFS
    error = 0.005
    plt.title(r'\textbf{%s}' % sample)
    pltname = 'figures/plt_pr_wear_'+sample
...
plot_data(df_wh,
          "comp_wh",
          list(range(0, 30, 5)), r'Покращення властивості, рази', (4,3))

ax=df_grav_comp.plot(kind="bar", fontsize=10, yticks=list(range(0,55,5)), figsize=(4,2.1))
filename = "figures/pr_" + "comp_grav"
for p in ax.patches:
    ax.annotate(str(p.get_height()), (p.get_x() * 0.98, p.get_height() * 1), fontSize=12)

plt.ylabel(r'Коефіцієнт ефективності, в.од.', size=9)
plt.xlabel('Зразок', size=12)
plt.xticks(rotation='horizontal', size=12)
...
plt.axes().set_aspect('auto')
plt.tight_layout(0)
plt.savefig("figures/pr_comp_grav" + ".pdf", format="pdf")
plt.clf()